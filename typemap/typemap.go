...
func (r *Registry) FileComments(file *descriptor.FileDescriptorProto) (DefinitionComments, error) {
	return commentsAtPath([]int{packagePath}, file), nil
}
	...
	for i, s := range file.Service {
		if s == svc {
			...
			for j, m := range s.Method {
				if m == method {
					path = append(path, serviceMethodPath, j)
					return commentsAtPath(path, file), nil
				}
			}
		}
...
func (m *MessageDefinition) descendants() []*MessageDefinition {
	...
	for i, child := range m.Descriptor.NestedType {
		path := m.path
		path = append(path, messageMessagePath, i)
...
func commentsAtPath(path []int, sourceFile *descriptor.FileDescriptorProto) DefinitionComments {
...
func pathEqual(path1 []int, path2 []int32) bool {
